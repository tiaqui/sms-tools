...
def computeODF(inputFile, window, M, N, H):
    ...
    fs = 44100
    energy_envelope = computeEngEnv(inputFile, window, M, N, H)
    binF = np.arange(N/2+1)*fs/N
    f1_inf = 1
    f2_inf = np.min(np.where(binF > 3000)) # first value in second band
    f1_sup = f2_inf - 1
    f2_sup = np.max(np.where(binF < 10000)) # last value in second band
    ODF = np.zeros(shape=(energy_envelope.shape[0], 2))
    for l in np.arange(energy_envelope.shape[0]-1)+1:
            ODF[l,0] = energy_envelope[l,0]/10 - energy_envelope[l-1,0]/10
            ODF[l,1] = energy_envelope[l,1]/10 - energy_envelope[l-1,1]/10
    ODF[:,0][ODF[:,0] < 0] = 0
    ODF[:,1][ODF[:,1] < 0] = 0
    return ODF