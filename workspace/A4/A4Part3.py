...
def computeEngEnv(inputFile, window, M, N, H):
    ...
    low = 10**(mX[:,f1_inf:f1_sup+1]/20)
    high = 10**(mX[:,f2_inf:f2_sup+1]/20)
    
    en_low = 10*np.log10(np.apply_along_axis(energy, 1, low))
    en_high = 10*np.log10(np.apply_along_axis(energy, 1, high))
    engEnv = np.zeros(shape=(mX.shape[0], 2))
    engEnv[:,0] = en_low
    engEnv[:,1] = en_high
    return engEnv