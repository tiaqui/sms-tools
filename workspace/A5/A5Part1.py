...
def minFreqEstErr(inputFile, f):
    ...
    t_loc = 0.5 # centered time location [s]
    max_e = 0.05 # maximum error in Hz
    f_range = (100, 2000)
    M = 0
    N = 0
    def peak_loc(x, M, N):
        xx = x[int(int(t_loc*fs - np.floor(M/2))):int(int(t_loc*fs) + np.floor((M+1)/2))]
        mX, pX = DFT.dftAnal(xx, w, N)
        ploc = UF.peakDetection(mX, t)
        iploc, ipmag, ipphase = UF.peakInterp(mX, pX, ploc)
        return iploc*fs/N
    for k in np.arange(np.int(np.floor((len(x)-1)/100))) + 1:
        M = 100*k + 1
        N = 2**np.arange(20)[np.min(np.where(2**np.arange(20)>M))]
